...
SELECT 
  `Region`,
  ROUND(100.0 * high / region_total, 1) AS `High income`,
  ROUND(100.0 * upper_mid / region_total, 1) AS `Upper middle income`,
  ROUND(100.0 * lower_mid / region_total, 1) AS `Lower middle income`,
  ROUND(100.0 * low / region_total, 1) AS `Low income`,
  ROUND(100.0 * other / region_total, 1) AS `Other`,
  100.0 AS `Total`
FROM region_counts
ORDER BY `Region`;
""")
...
income_totals AS (
  ...
  FROM income_region_counts
  GROUP BY `Income Group`
)
SELECT 
  ir.`Income Group`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'East Asia & Pacific' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `East Asia & Pacific`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'Europe & Central Asia' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `Europe & Central Asia`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'Latin America & Caribbean' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `Latin America & Caribbean`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'Middle East & North Africa' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `Middle East & North Africa`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'North America' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `North America`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'South Asia' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `South Asia`,
  ROUND(100.0 * SUM(CASE WHEN ir.`Region` = 'Sub-Saharan Africa' THEN ir.country_count ELSE 0 END) / it.total, 1) AS `Sub-Saharan Africa`,
  100.0 AS `Total`
FROM income_region_counts ir
JOIN income_totals it ON ir.`Income Group` = it.`Income Group`
GROUP BY ir.`Income Group`, it.total
ORDER BY ir.`Income Group`;
""")
