...
SELECT 
  ...
  (SUM(CASE WHEN `Income Group` IS NOT NULL THEN 1 ELSE 0 END)) AS `Row Total`
FROM wdi_country
GROUP BY `Region`
ORDER BY `Region`;
""")
...
ON ac.`Region` = existing.`Region` AND ac.`Income Group` = existing.`Income Group`
...
SELECT 
  b.`Region`,
  ROUND(SUM(CASE WHEN `Income Group` = 'High income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `High income`,
  ROUND(SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Upper middle income`,
  ROUND(SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Lower middle income`,
  ROUND(SUM(CASE WHEN `Income Group` = 'Low income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Low income`,
  ROUND(SUM(CASE WHEN `Income Group` IS NULL THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Other`,
...
run_sql_and_return_html(cnx,"""
WITH base AS (
  SELECT COALESCE(`Income Group`, 'Other') AS `Income Group`, COUNT(*) AS country_count
  FROM wdi_country
  GROUP BY `Income Group`
),
grand_total AS (
...
FROM base b
ORDER BY FIELD(`Income Group`, 'High income', 'Upper middle income', 'Lower middle income', 'Low income', 'Other');
""")
