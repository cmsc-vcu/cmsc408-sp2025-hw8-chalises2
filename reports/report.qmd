...
LEFT JOIN 
  (SELECT DISTINCT Region, `Income Group` FROM wdi_country WHERE Region IS NOT NULL AND `Income Group` IS NOT NULL) w
ON r.Region = w.Region AND i.`Income Group` = w.`Income Group`
WHERE 
  w.Region IS NULL
ORDER BY 
