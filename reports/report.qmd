...
SELECT 
  ...
  SUM(CASE 
      WHEN `Income Group` IN ('High income', 'Upper middle income', 'Lower middle income', 'Low income') THEN 1 
      ELSE 0 
    END) AS `Row Total`
FROM wdi_country
GROUP BY `Region`
...
SELECT 
    ...
    'Missing' AS `Status`
FROM 
    region_income_pairs p
LEFT JOIN 
    existing_region_income e
ON 
    p.`Region` = e.`Region` AND p.`Income Group` = e.`Income Group`
WHERE 
    e.`Region` IS NULL
ORDER BY 
...
SELECT 
    b.`Region`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'High income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `High income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Upper middle income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Upper middle income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Lower middle income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Lower middle income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Low income' THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Low income`,
    ROUND(SUM(CASE WHEN b.`Income Group` IS NULL THEN country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Other`,
    ROUND(SUM(country_count) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Row Total`
FROM 
    base b
GROUP BY 
    b.`Region`
...
SELECT 
    b.`Income Group`,
    b.`country_count`,
    (SELECT total_countries FROM grand_total) AS total_countries,
    ROUND((b.`country_count` / (SELECT total_countries FROM grand_total)) * 100, 2) AS percent_of_total
FROM base b
ORDER BY FIELD(`Income Group`, 'High income', 'Upper middle income', 'Lower middle income', 'Low income', NULL);
""")
