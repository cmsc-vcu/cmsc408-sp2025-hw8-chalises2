...
SELECT 
    b.`Region`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'High income' THEN b.country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `High income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Upper middle income' THEN b.country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Upper middle income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Lower middle income' THEN b.country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Lower middle income`,
    ROUND(SUM(CASE WHEN b.`Income Group` = 'Low income' THEN b.country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Low income`,
    ROUND(SUM(CASE WHEN b.`Income Group` IS NULL THEN b.country_count ELSE 0 END) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Other`,
    ROUND(SUM(b.country_count) / (SELECT total_countries FROM grand_total) * 100, 2) AS `Row Total`
FROM 
    base b
GROUP BY 
    b.`Region`
